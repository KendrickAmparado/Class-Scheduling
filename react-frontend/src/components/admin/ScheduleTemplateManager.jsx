...
const ScheduleTemplateManager = ({ 
  ...
  const fetchTemplates = React.useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_BASE || 'http://localhost:5000'}/api/schedule-templates?course=${course}&year=${year}`);
      if (res.data.success) {
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_API_BASE || 'http://localhost:5000'}/api/schedule-templates`, {
  ...
  const handleDeleteTemplate = (templateId, templateName) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          const res = await axios.delete(`${process.env.REACT_APP_API_BASE || 'http://localhost:5000'}/api/schedule-templates/${templateId}`);
          if (res.data.success) {
